  ...
  try {
    if (req.session.admin) {
      ...
      const monthlyDataArray = await orderModel.aggregate([
        ...
        { $group: { _id: { $month: "$createdAt" }, sum: { $sum: "$total" } } },
      ]);
      ...
      //
      categoryWiseOrder = await orderModel.aggregate([
        { $unwind: "$product" },
        {
          $group: {
            _id: "$product.category",
            totalItemPrice: { $sum: "$product.itemprice" },
          },
        },
      ]);
      categoryWiseSale = await orderModel.aggregate([
        { $match: {} },
        { $unwind: "$product" },
        {
          $group: {
      ...
      const categoryWiseSaleWithName = categoryWiseSale.map((sale) => {
        const category = categoryData.find(
          (cat) => cat._id.toString() === sale._id.toString()
        );
        return {
...
export async function getOrderDetails(req, res) {
  try {
    ...
    let orderData = await orderModel.find({ orderId: orderId }).lean();
...
export async function sideImagesDel(req, res) {
  try {
    ...
    let imageSource = await productModel.updateOne(
      { "sideImages.filename": filename },
      { $pull: { sideImages: { filename: filename } } }
    );
    // await couponModel.findByIdAndUpdate(id, { $set: { list: false } });
    res.json({ data: true });
    //
